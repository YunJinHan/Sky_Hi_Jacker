...
Client.prototype.hack = function(current_lat,current_lon,dest_lat,dest_lon) {
  console.log("HACKING!!");
  ...
  function calBearingAngle(lat1, lon1, lat2, lon2) {
      var current_lat_radian = deg2rad(lat1);
      var current_lon_radian = deg2rad(lon1);

      var destination_lat_radian = deg2rad(lat2);
      var destination_lon_radian =deg2rad(lon2);

      var radian_distance_tmp1 = Math.acos(current_lat_radian) * Math.sin(destination_lat_radian);
      var radian_distance_tmp2 = Math.cos(current_lat_radian) * Math.cos(destination_lat_radian) * Math.cos(current_lon_radian - destination_lon_radian);
      var radian_distance = radian_distance_tmp1 + radian_distance_tmp2;

      var radian_bearing_tmp1 = Math.sin(destination_lat_radian) - Math.sin(current_lat_radian) * Math.cos(radian_distance);
      var radian_bearing_tmp2 = Math.cos(current_lat_radian) * Math.sin(radian_distance) ;
      var radian_bearing = Math.acos(radian_bearing_tmp1 / radian_bearing_tmp2);
      // 현재 좌표부터 목적지로의 이동 방향

      var true_bearing = 0;
      true_bearing = rad2deg(radian_bearing);
      if (Math.sin(destination_lon_radian - current_lon_radian) < 0) {
          true_bearing = 360 - true_bearing;
      }
      return true_bearing.toFixed(2);
  }

  var distance = calDistance(current_lat,current_lon,dest_lat,dest_lon);
  var degree = calBearingAngle(current_lat,current_lon,dest_lat,dest_lon)*1;

  var data = [distance,degree];

  var self = this;

  self._hack = true;
  self._hackData = data;
  self._destLat = dest_lat;
  self._destLong = dest_lon;
